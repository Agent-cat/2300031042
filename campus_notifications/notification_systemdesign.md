...
### 1. Shortcomings of Current Code

- **Blocks Everything:** Running 50k operations in a single synchronous loop will timeout the HTTP request.
- **Cascading Failures:** If `send_email` fails for student #200, the loop crashes, leaving the remaining 49,800 students without any notification.
- **DB & API Abuse:** 50,000 individual database inserts will crash/lock the database, and the Email API will block us for rate-limiting.

### 2. How to Recover & Redesign

- **What now?** Check logs/DB to find which student ID failed, then run a one-time recovery script targeting only the remaining IDs.
- **Separation of Concerns:** Do **not** save to DB and send emails in the same synchronous flow. Email API is an unreliable external call; DB is fast and internal.
- **The Solution:** Push jobs to a message queue (like Redis/BullMQ) and let background workers process them asynchronously with automatic retry logic.
...
async function notify_all(student_ids, message) {
  ...
  return { success: true, queued: student_ids.length };
}

notificationQueue.process("deliver-notification", async (job) => {
  const { student_id, message } = job.data;

  await save_to_db(student_id, message);
  await push_to_app(student_id, message);

  await emailQueue.add({ student_id, message }, { attempts: 3, backoff: 5000 });
});
...
I had calculated a total score for each notification by adding a static type weight (Placement > Result > Event) to a time-based recency score that decays as the notification ages. The list is then sorted by this total score in descending order to return the top N highest-priority items.
